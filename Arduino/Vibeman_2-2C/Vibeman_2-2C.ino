...
#define BRIGHTNESS 200 //Subject to change, limits current that the LEDs draw
...
#define V_LONG_PRESS_MS 1500 //ms for a very long press, which turns the device off
...
//=======Global Settings=============================
//DIP switch options:
uint8_t MOT_MAX =   255; //By default, motor speed only ever reaches 179. Alternative is 255
bool SW2 =        false;
bool SW3 =        false;
bool SW4 =        false;
#define MOT_MIN 64  // Motor PWM minimum.  It needs a little more than this to start.
...
void beep_motor(int f1, int f2, int f3){
  ...
  delay(250);
  analogWriteFrequency(MOTPIN, MOTFREQ);
...
uint8_t check_button(){
  ...
  if (thisBtn == HIGH && millis() - keyDownTime > V_LONG_PRESS_MS) {
